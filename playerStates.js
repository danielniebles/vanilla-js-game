...
export class Sitting extends State {
  ...
  handleInput(input) {
    if (input.includes(controls.LEFT) || input.includes(controls.RIGHT)) {
      this.game.player.setState(states.RUNNING, 1);
    } else if (input.includes(controls.ROLL)) {
...
export class Running extends State {
  ...
  handleInput(input) {
    ...
    if (input.includes(controls.DOWN)) {
      this.game.player.setState(states.SITTING, 0);
    } else if (input.includes(controls.UP)) {
...
export class Jumping extends State {
  ...
  handleInput(input) {
    if (this.game.player.vy > this.game.player.weight) {
      ...
      this.game.player.setState(states.ROLLING, 2);
    } else if (input.includes(controls.DOWN)) {
...
export class Rolling extends State {
  ...
  handleInput(input) {
    ...
    if (!input.includes(controls.ROLL) && this.game.player.onGround()) {
      ...
      input.includes(controls.UP) &&
      this.game.player.onGround()
    ) {
      this.game.player.vy -= 27;
    } else if (input.includes(controls.DOWN) && !this.game.player.onGround()) {
      this.game.player.setState(states.DIVING, 0);
    }
  }
}
